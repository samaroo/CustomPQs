...
class UnorderedFastPQ : public Eecs281PQ<TYPE, COMP_FUNCTOR> {
 ...
 public:
  ...
  explicit UnorderedFastPQ(COMP_FUNCTOR comp = COMP_FUNCTOR())
      : BaseClass{comp}, extreme{UNKNOWN} {}  // UnorderedFastPQ()
  ...
  UnorderedFastPQ(InputIterator start, InputIterator end,
      ...
      : BaseClass{comp},
        data{start, end},
        extreme{UNKNOWN} {}  // UnorderedFastPQ()

  // Description: Destructor doesn't need any code, the data vector will
  //              be destroyed automaticslly.
  virtual ~UnorderedFastPQ() {}  // ~UnorderedFastPQ()

  // Description: The only thing needed is to mark that we no longer know the
  //              most extreme element.
  // Runtime: O(1)
  virtual void updatePriorities() { extreme = UNKNOWN; }  // updatePriorities()
  ...
  virtual void pop() {
    ...
    if (extreme == UNKNOWN) findExtreme();
 ...
 private:
  ...
  void findExtreme() const {
    ...
    for (size_t i = 1; i < data.size(); ++i)
      if (this->compare(data[index], data[i])) index = i;
